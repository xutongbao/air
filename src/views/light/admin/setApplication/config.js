...
//添加编辑查看对话框表单字段
const getModalFields = ({ dataSource, type, initValues, formComponentName }) => {
  ...
  return (
    <>
      ...
      >
        <Select placeholder="请选择" getPopupContainer={() => document.getElementById('m-set-application-modal-form')}>
      ...
      >
        <Select placeholder="请选择" getPopupContainer={() => document.getElementById('m-set-application-modal-form')}>
          {getRenderFunArr().filter(item => item.formComponentNameArr.includes(formComponentName)).map((item) => (
